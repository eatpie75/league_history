...
urlpatterns=patterns('lol.views',
	...
	url(r'^summoner/(?P<region>NA|EUW|EUNE)/(?P<account_id>\d{1,12})/(?P<slug>[-\w]*?)/inventory/', 'view_summoner_inventory', name='view_summoner_inventory'),
	url(r'^summoner/(?P<region>NA|EUW|EUNE)/(?P<account_id>\d{1,12})/(?P<slug>[-\w]*?)/', 'view_summoner', name='view_summoner'),
	url(r'^summoner/(?P<region>NA|EUW|EUNE)/(?P<account_id>\d{1,12})/', 'view_summoner_redirect', name='view_summoner_redirect'),
	url(r'^champions/(?P<champion_id>\d{1,12})/(?P<champion_slug>[-\w]*?)/items/$', 'view_champion_items', name='view_champion_items'),
