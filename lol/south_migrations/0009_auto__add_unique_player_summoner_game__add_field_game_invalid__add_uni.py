...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'Player', fields ['summoner', 'game']
        db.create_unique('lol_player', ['summoner_id', 'game_id'])

        # Adding field 'Game.invalid'
        db.add_column('lol_game', 'invalid',
                      self.gf('django.db.models.fields.BooleanField')(default=False),
                      keep_default=False)

        # Adding unique constraint on 'Game', fields ['game_id', 'region']
        db.create_unique('lol_game', ['game_id', 'region'])

        # Adding unique constraint on 'SummonerRating', fields ['summoner', 'game_mode', 'game_map']
        db.create_unique('lol_summonerrating', ['summoner_id', 'game_mode', 'game_map'])

        # Adding unique constraint on 'Summoner', fields ['region', 'account_id']
        db.create_unique('lol_summoner', ['region', 'account_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Summoner', fields ['region', 'account_id']
        db.delete_unique('lol_summoner', ['region', 'account_id'])

        # Removing unique constraint on 'SummonerRating', fields ['summoner', 'game_mode', 'game_map']
        db.delete_unique('lol_summonerrating', ['summoner_id', 'game_mode', 'game_map'])

        # Removing unique constraint on 'Game', fields ['game_id', 'region']
        db.delete_unique('lol_game', ['game_id', 'region'])

        # Removing unique constraint on 'Player', fields ['summoner', 'game']
        db.delete_unique('lol_player', ['summoner_id', 'game_id'])

        # Deleting field 'Game.invalid'
        db.delete_column('lol_game', 'invalid')
    ...
    models = {
        ...
        'lol.summoner': {
            ...
            'time_updated': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 9, 14, 0, 0)'}),
